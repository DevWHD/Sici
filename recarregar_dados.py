...
for chave_principal, item_data in dados_originais.items():
    ...
    # Pegar telefones (todos disponíveis)
    telefones = []
    if 'comunicacoes' in dados and dados['comunicacoes']:
        for comm in dados['comunicacoes']:
            if comm.get('valor') and 'Telefone' in comm.get('tipo', ''):
                telefones.append(comm.get('valor', ''))
...
# Mostrar resumo
print(f"\n✓ {len(dados_transformados)} registros processados com dados completos")
